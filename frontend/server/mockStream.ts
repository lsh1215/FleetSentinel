 ...
 * 100ms 프레임으로 묶어 흘린다. 그래야 프론트가 실제와 같은 부하(초당 약 5,100 레코드)를
 * 만난다.
 *
 * 묶는 것이 **브라우저 구간의 결정**이라는 점이 중요하다. 차량→클라우드 구간에서는
 * 애플리케이션 배치를 폐기했다 — 축적 창 안에서 죽으면 그 레코드는 세상에서 사라진다.
 * 여기서 놓친 것은 저장 계층에 남아 있고 `Last-Event-ID`로 다시 받을 수 있으므로 같은
 * 기법이 다른 의미를 갖는다. (docs/frontend-tech-notes.md Q3-1)
 *
 * 재현하는 백엔드 계약:
