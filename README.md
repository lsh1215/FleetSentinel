...
아래 수치는 전부 **실측값**입니다 (nuScenes mini 10장면, 196.5초 전수 측정).
...
**대역폭은 58배 차이인데 메시지 건수는 오히려 신호가 8배 많습니다.** 하나의 파이프라인으로
둘 다 처리하려는 순간 설계가 무너집니다. 그리고 차량 1대의 27.15 MB/s는 LTE 실효 대역폭
(약 12.5 MB/s)을 넘어서 **한 대조차 연속 업로드가 불가능**합니다.

이 두 사실이 아키텍처 전체를 결정합니다.

### 원천 데이터의 제약 (정직하게)

| 항목 | 값 |
|---|---|
| 수집 차량 | **2대** (`n015`, `n008`) — fleet이 아닙니다 |
| 총 주행시간 | 5.4시간 (1,000장면 × 20초) |
| **연속 구간 상한** | **20초** — 같은 차량·같은 날 장면도 2~8분씩 떨어져 있어 이어붙일 수 없습니다 |

nuScenes는 연속 주행 로그가 아니라 **긴 주행에서 "흥미로운 20초"만 골라낸 큐레이션
데이터셋**입니다. 정확히 말하면 *5.4시간짜리 주행이 아니라 20초짜리 주행 1,000개*입니다.

따라서 이 문서의 **"차량 N대"는 동시 스트림 N개**를 뜻하며, 서로 다른 실차량 N대의
데이터가 아닙니다. 처리량·유실·중복 검증에는 무해하지만(파이프라인은 스트림의 출처를
구분하지 못합니다), 관제 화면에서는 20초마다 차량이 점프하는 것이 보입니다.
연속 데이터가 필요해지면 **nuPlan**(같은 팀, 수 분 단위 로그)이 상위 원천입니다.
...
| 게이트 | 결과 |
|---|---|
| 좌표 변환 계약 (pytest) | **30건 통과** |
| MCAP 유효성 | 3장면 × 9항목 — 인덱스·내장 스키마·구간 랜덤 액세스 |
| **좌표 체인 종단** | 박스 안 LiDAR 포인트 대조 **31,911개 오차 0** |
| Kafka HA | 브로커 하드 kill → 리더 재선출 → **published 5600 = consumed 5600** |
| 인프라 | `make smoke` 전 서비스 healthy |

좌표 종단 검증이 결정적입니다 — 3D 박스 안의 LiDAR 포인트를 직접 세어 nuScenes 라벨과
대조했을 때 오차 0이 나오려면 `센서 프레임 → 캘리브레이션 → ego 자세 → 글로벌 → 박스 로컬`
**다섯 단계가 전부** 맞아야 합니다.
