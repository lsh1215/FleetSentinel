...
> **무엇을 / 무엇을 위해 / 어떻게 쓰는가** 를 정리한 데이터 설계 문서.
> `docs/sdd.md` v2.0(§6 Data Design)을 **대체**한다. SDD v3.0 개정의 입력 문서이며,
> 이 문서와 SDD가 충돌하면 **이 문서가 우선**한다.
...
| CAN bus 확장 | 별도 다운로드. `pose`(50Hz), `ms_imu`, `steeranglefeedback`, `vehicle_monitor`, `zoesensors`, `zoe_veh_info` |
...
**이 표가 이 문서 전체에서 가장 중요하다.** 세 계층의 대역폭이 약 550배 차이나므로
"모든 데이터를 Kafka에 Avro 레코드로" 라는 v2.0 모델은 여기서 성립하지 않는다.

| 계층 | 내용 | 주기 | **차량 1대당 대역폭** | 전송 경로 |
|---|---|---|---|---|
| **① 신호(Signal)** | ego pose, IMU, 조향각, 속도, 가속도, 자율주행 모드 | 2–50Hz | **~수십 KB/s** | 경량 → Kafka |
| **② 인지 산출(Perception)** | 3D 박스, 트랙 ID, 클래스, 가시성, 포인트 수 | 2Hz(키프레임) | **~수십 KB/s** | 경량 → Kafka |
| **③ 원시 센서(Raw)** | 카메라 6대, LiDAR, 레이더 5대 | 12–20Hz | **~27.5 MB/s** | **중량 → 오브젝트 스토리지** |

**③의 산출 근거**: 550GB ÷ 1,000 scene = scene당 약 550MB, ÷ 20초 = **초당 약 27.5MB**.
차량 50대를 동시 재생하면 **1.4 GB/s** — Kafka로 나를 수 있는 양이 아니다.
