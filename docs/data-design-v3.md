...
|---|---|---|---|---|---|
| **① 신호(Signal)** | CAN 버스, IMU, 조향, 차량 자세 | 2–955Hz | **1,466** | **230 KB/s** | 경량 → Kafka |
...
| 채널 | 형식 | Hz | 필드 | 대역폭 |
|---|---|---|---|---|
| **`zoesensors`** | JSON | **955.0** | `brake_sensor`, `steering_sensor`, `throttle_sensor` | 123.5 KB/s |
| `ego_pose` | JSON | 158.8 | `translation`, `rotation`(쿼터니언), `timestamp` | 52.7 KB/s |
| `ms_imu` | JSON | 99.8 | `linear_accel`, `q`, `rotation_rate` | 17.5 KB/s |
| `zoe_veh_info` | JSON | 99.6 | 차량 정보 8종 | 15.2 KB/s |
| `steeranglefeedback` | JSON | 98.2 | `value` (rad) | 8.1 KB/s |
| `pose` | JSON | 49.8 | `pos`, `vel`, `accel`, `orientation`, `rotation_rate` | 12.2 KB/s |
| `vehicle_monitor` | JSON | 2.0 | `vehicle_speed`, `yaw_rate`, `steering`, `brake`, `throttle`, `gear_position`, `rear_left_rpm`, `rear_right_rpm`, `battery_level`, 방향지시등 | 0.5 KB/s |
| **합계** | | **1,466.1** | | **229.7 KB/s** |

> ⚠️ **`zoesensors`가 955Hz다.** 샘플 간격이 **0.25ms**까지 내려간다 — 페달·조향 원시 센서를
> 밀리초 이하로 뽑는다. 이 한 채널이 신호 계층 메시지의 **65%** 를 차지한다.
> 설계 초안은 `ms_imu` 100Hz가 최고인 줄 알았으나 실제로는 10배 높은 채널이 있었다.
...
> ⚠️ **현존 결함 — 스윕(sweep) 누락.** 현재 변환기(`ingestion/scripts/convert_scenes.py`)는
> **키프레임 센서 데이터만** MCAP에 담는다. 실측하면 `sample_data` 31,206건 중 키프레임은
> 4,848건(**15.5%**)이고 바이트로는 **13.6%** 다. 즉 원시 센서의 **86%가 빠져 있다**.
>
> | | 데이터 | 대역폭 |
> |---|---|---|
> | 전체 (키프레임 + 스윕) | 5.34 GB | 27.15 MB/s |
> | 현재 MCAP (키프레임만) | 0.73 GB | 3.7 MB/s |
>
> 이 절의 "무손실 원본" 주장과 정면으로 어긋난다. **P2 착수 전 수정 대상**이며
> [SDD](sdd.md) §4.2 L-10에 한계로 등재돼 있다. 수정하면 장면당 MCAP이 약 49MB →
> 약 360MB로 커지므로, `log-segment`의 세그먼트 분할 단위(클립 통째 vs 시간 구간)를
> 먼저 정해야 한다.
...
| **R-V3-6** | **MCAP 스윕 누락 — 원시 86% 유실** | ❌ **미해결** | §9.1. P2 선결 과제 |
