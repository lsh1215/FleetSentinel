...
>
> **표기 규약** — "차량 N대"는 항상 **동시 스트림 N개**다. 실제 수집 차량은 2대다(§2.1.2).
...
| **수집 차량** | **2대** (`n015`, `n008`) — mini의 log 8건 전수 확인. §2.1.1 |
| **총 주행시간** | 1,000 × 19.5초 = **약 5.4시간** |
...
#### 2.1.1 두 가지 규모 제약 — 총량과 연속성

설계 판단에 직접 영향을 주므로 분리해 적는다. **둘은 다른 문제다.**

| | 값 | 조절 가능한가 |
|---|---|---|
| **총량** | 5.4시간 | ✅ 가상 차량 수로 나뉜다(§2.1.2) |
| **연속 구간** | **20초** | ❌ **못 늘린다** |

**연속 구간이 20초인 이유**는 nuScenes가 연속 주행 로그가 아니라 **긴 주행에서 "흥미로운
20초"만 골라낸 큐레이션 데이터셋**이기 때문이다. 같은 `log`(같은 차량·같은 날)의 장면들조차
이어지지 않는다 — mini 실측:
...
인접 장면 간격이 최소 120.5초다. **20초 이내로 붙은 쌍은 0건.** 이어붙일 수 없다.

정확한 정신 모델은 이렇다.

> **5.4시간짜리 주행이 아니라, 20초짜리 주행 1,000개다.**
...
| 용도 | 20초 제약이 문제인가 |
|---|---|
| 처리량·유실 0·중복 제거 검증 | ✅ 무해 — 이벤트 개수와 동시성의 문제이고 연속성과 무관 |
| 장시간 안정성(메모리 누수, 상태 누적) | ✅ 무해 — 반복이라도 시간만 길면 된다 |
| 관제 화면 데모 | ⚠️ **20초마다 차량이 점프하는 게 보인다** |
| 궤적 예측·장기 거동 학습 | ❌ 연속 필요 |
| 지오펜스 체류 등 시간 누적 분석 | ❌ 연속 필요 |

이 프로젝트는 위쪽 두 항목이 주 목표이므로 **20초 클립으로 충분하다.** 또한 트리거 클립이
실제 fleet의 동작 방식이므로(§5 참고, 차량 1대의 27 MB/s는 연속 업로드가 불가능하다)
20초 구조가 현실을 반영한다. 다만 **관제 데모의 점프는 실재하는 약점**이다.

연속 데이터가 필요해지면 상위 원천은 **nuPlan**이다 — 같은 팀(Motional), 1,200~1,500시간,
약 15,000개의 **수 분 단위 연속 로그**. 단 원시 센서가 있는 건 120시간(전체의 10%, ~16TB)뿐이다.

#### 2.1.2 "N대"는 동시 스트림 N개이고, 실제 차량 N대가 아니다

원천 차량이 2대이므로 **서로 다른 실제 차량 N대의 데이터는 존재하지 않는다.** 재생기가
장면에 가상 차량 id를 붙여 N개 스트림을 동시에 흘리는 것이며, 문서에서 "N대"는 항상
이 뜻이다(`AV-000N` 접두어로 표기).

파이프라인 관점에서 **실제 N대와 구별되지 않는 것** — 서로 다른 키 N개로 파티셔닝, 시간축
뒤섞임, 총 처리량 N배, 동시성·백프레셔·순서·중복 제거가 동일하게 걸린다.

**구별되는 것** — 지리 분포(모든 차량이 같은 4개 지역), 내용 독립성(다른 차량이 같은 물리
차량의 주행을 재생), 차량 간 상관(실제 fleet은 서로를 보고 도로를 공유하나 여기선 독립).

앞의 세 가지는 처리량·유실·중복 검증에 무해하고, 관제 화면에는 유해하다. 재생 모드
선택으로 다루며 상세는 [`pipeline-notes-provisional.md`](pipeline-notes-provisional.md)에 있다.
...
**이 표가 이 문서 전체에서 가장 중요하다.** 대역폭은 58배 차이인데 **메시지 건수는 오히려
신호가 8배 많다.** 이 비대칭 때문에 "모든 데이터를 하나의 Kafka 레코드로"라는 v2.0 모델이
여기서 성립하지 않는다.
...
배치 없이 원시 레이트로 보내면 스트림 500개에서 647,610 msg/s이고 레코드가 평균 342바이트에
불과해 브로커 오버헤드가 지배적이다. **100ms 창 배치**를 적용하면 차량당 9.8 msg/s(메시지당
약 132 레코드, 43.4KB)로 줄어 스트림 500개에서 **4,925 msg/s**가 된다.
상세는 [`pipeline-notes-provisional.md`](pipeline-notes-provisional.md).
