...
**자율주행 멀티모달 센서 데이터 수집·실시간 관제 플랫폼**이다.

한 문장으로 줄이면 이렇다.

> **차량의 고주파 신호와 대용량 센서 원본을 유실 없이 수집하고, 이상 상태와 관련 구간을 운영자가 바로 확인할 수 있게 하는 시스템이다.**

이 프로젝트의 범위는 차량에서 서버까지 데이터를 전달하는 수집 계층, 실시간 스트림 처리,
관제용 저장·조회, 경보 전달이다.

### 1.2 무엇을 다루는가

차량 한 대가 만들어내는 데이터는 성격이 완전히 다른 **세 계층**으로 갈린다 —
**① 신호**(CAN·IMU·조향·자세), **② 객체 메타데이터**(3D 박스·트랙), **③ 원시 센서**(카메라 6대·
LiDAR·레이더 5대).

**대역폭은 원시가 58배 무거운데 메시지 건수는 신호가 8배 많다.** 이 비대칭이 이 시스템
설계 전체를 결정한다 — 하나의 파이프라인으로 둘 다 처리하려는 순간 무너진다.

> 계층별 실측 수치(Hz·크기·형식·채널별 상세)는 **[데이터 설계 §1, §4](data-design.md)** 이
> 정본이다. 이 문서는 수치를 다시 적지 않고 설계 판단만 다룬다.

### 1.3 데이터 원천

**nuScenes** — 보스턴·싱가포르 실도로에서 수집된 공개 자율주행 데이터셋. 합성이 아니라
실측이며, 원래 타임스탬프대로 재생해 실시간 스트림으로 흘린다. 공개 로그 재생은 실제 차량
연동 없이 수집·처리·관제 경로를 검증하기 위한 입력이다.
...
![FleetSentinel 시스템 아키텍처](assets/fleetsentinel-architecture.png)

그림은 차량 센서와 재생기에서 시작해 수집 서비스, Kafka, Flink, ClickHouse, API, React
콘솔로 이어지는 전체 흐름을 보여준다. 고주파 텔레메트리는 `telemetry.records`로 흐르고,
원본 MCAP 클립은 오브젝트 스토리지에 저장한 뒤 참조로 연결한다. Flink가 생성한 ODD 알림은
`fleet.alerts`를 통해 API와 SSE 클라이언트로 전달된다.
...
Flink (exactly-once)
 ...
 ├─ 파생        ENU 미터 → WGS84 위경도
 ├─ ODD         ego_pose 전이 + processing-time 지속 타이머
 │
 ├──────────────▶ Kafka fleet.alerts ─▶ Spring Boot ─▶ /api/alerts SSE
 ▼
ClickHouse
 ├─ 신호·객체 메타데이터 시계열
...
온보드 링버퍼         ← 연속 기록, 덮어쓰기
 │
 ▼  트리거 발생 (급제동 · ODD 이탈 · 센서 단절 · 운전자 개입)
...
센서 프레임, 객체 박스는 글로벌 프레임이라 정렬되지 않는다([데이터 설계 §7.2](data-design.md)).
...
│ 객체 요약    n_objects · n_zero_lidar · max_speed         │  ← 경량 경로에서
│ blob_uri     s3://…/scene-0061.mcap                      │  ← 중량 경로에서
└──────────────────────────────────────────────────────────┘
        │
        ├─▶ 사건 검색: 조건으로 조회 → blob_uri로 원본 재생
        └─▶ 관제 검토: 경보 발생 전후의 센서 원본 확인
```
...
| 스트림 처리 | **Apache Flink** | **2.0.0** | Docker 이미지와 Maven 의존성을 같은 버전으로 고정. **PyFlink는 기각**(§4.1 A-13) — 커넥터·API 조합 때문이며 성능 문제는 아니다 |
| 원시 로그 저장 | 오브젝트 스토리지 + **MCAP** | — | **S3 호환 API로 접근한다** — 로컬은 MinIO, 배포 시 GCS의 S3 호환(XML) 엔드포인트다. `s3://`는 벤더가 아니라 **접근 프로토콜**을 가리킨다. MCAP은 ROS 2 기본 bag 포맷 |
...
| **수집 게이트웨이** | **Spring Boot + Spring gRPC** | **4.0.x / Spring gRPC 1.0.3** | 업링크를 종단해 Kafka에 쓰는 **애플리케이션 서비스**다 — LB·리버스 프록시가 아니다(§4.1 A-14). Spring gRPC 스타터는 1.0.3까지 발행됐고 Spring Boot 4.0.x 정렬이다(1.1.x는 core만 있고 Boot 스타터가 없다) |
...
| 실시간 푸시 | **SSE** | — | `/api/stream`은 텔레메트리 재생, `/api/alerts`는 Kafka 전이 이벤트. 서버→클라이언트 단방향이면 충분 |
| 탐색 도구 | Python | 3.12 | `exploration/` — 파이프라인 아님 |

#### Java 버전이 모듈마다 다르다

Spring Boot 4 API는 Java 21로 실행하고, Docker의 Flink 2.0.0 이미지와 잡은 Java 17로 맞춘다.

| 모듈 | Java | 근거 |
|---|---|---|
| Spring Boot API | **21** (LTS) | SB4 지원 범위 안, 라이브러리 생태계 성숙. Java 25도 LTS이고 지원되나 성숙도를 보고 21 채택 |
| **수집 게이트웨이** | **21** (LTS) | API와 같은 런타임으로 맞춘다 — 둘 다 Spring Boot 4.0.x라 버전을 나눌 이유가 없다 |
...
| ~~Kibana~~ | 자체 대시보드로 대체 |
| ~~BigQuery / 웨어하우스~~ | §4.1 **A-2** — 현재 관제 조회에 별도 분석 웨어하우스가 필요하지 않음 |
| ~~Apache Iceberg~~ | 관제용 시계열 조회에는 테이블 스냅샷이 필요하지 않고 카탈로그 운영이 추가되므로 제외. ClickHouse 싱크 중복은 멱등 upsert로 처리한다(S-7, L-14) |
...
| P2 | 스키마 확정 · **레코드 단위** 재생기 → Kafka | 구현 |
| P3 | Flink 파이프라인 · ClickHouse 적재 · ODD 이탈 판정 | 구현, 종단 측정 대기 |
| P5 | Spring Boot API — 텔레메트리/알림 SSE | 구현, 종단 측정 대기 |

Kafka→Flink→ClickHouse와 `fleet.alerts`→API→SSE 코드는 구현했다. 단위·계약 테스트는 통과했지만
종단 탐지 지연, checkpoint 복구, 부하 수치는 아직 측정하지 않았다.

인프라는 §1.5 결정에 맞춰 정리됐다 — Elasticsearch·Kibana를 제거하고 ClickHouse 26.3 LTS를
투입해 기동·질의·지리 함수를 실측 확인했다. Spring Boot API와 alert SSE도 구현했다.
...
| R-4 | 노트북 한 대(Apple Silicon)에서 전 구간이 돌아야 한다 |
...
- 차량당 실측치와 스케일링 함수 → **[데이터 설계 §4.4](data-design.md)**
...
(채널별 실측 → [데이터 설계 §4](data-design.md))
...
943.8Hz부터 2.0Hz까지 **472배** 범위에 걸쳐 있고([데이터 설계 §4.2](data-design.md)), 각자 다른 클럭을 쓴다. 어느 시점의
...
센서마다 좌표 프레임이 다르다. LiDAR는 센서 프레임, 객체 박스는 글로벌 프레임이다.
...
#### P-8. 20초 클립이라 지속 부하를 만들 수 없다 (R-4)

전체 재생시간이 mini 195초, full로 가도 5.4시간이다. 루프하면 같은 레코드가 다시
들어오는데, 새 정체성을 주면 dedup을 검증할 수 없고 유지하면 전부 삼켜진다.

#### P-9. 게이트웨이를 뒀는데 문지기가 없다 (R-1)

내부 버스를 인터넷에 노출하지 않으려고, 그리고 차량 수천 대에 Kafka ACL을 발급·폐기하는 것이
비현실적이라 수집 게이트웨이를 뒀다([검토](ingestion-design-review.md) §4.6). 그런데 **노출면을
하나로 줄였을 뿐 그 하나를 잠그지 않으면 아무 클라이언트나 Kafka에 쓸 수 있다.**

그리고 인증만으로는 부족하다. `vehicle_id`가 파티션 키이자 dedup 키인데(S-7), 그 값을
**클라이언트가 주장**하면 남의 차량 파티션에 쓸 수 있다. 이 시스템에서 그 결과는 데이터 오염이
아니라 **표적 유실**이다 — 남의 `seq`를 밀어 정상 레코드를 윈도우 밖으로 떨어뜨린다.
...
**트리거(급제동·충돌·운전자 개입·ODD 이탈)가 걸린 앞뒤 20~30초만 업로드**한다.
...
ClickHouse에는 **ENU 원본과 파생한 `lat`/`lon`을 함께** 둔다. 원본 좌표를 남겨야 변환
로직을 바꾸거나 검증할 때 다시 계산할 수 있다.
...
> 후자는 그것을 가리키는 2KB 미만 레코드다. 구분은 [데이터 설계 §5.3](data-design.md).
...
| ack | **Cumulative Acknowledgement(CACK)** — 최고 연속 `seq`. 128개 또는 200ms 중 먼저, 차량당 10.1 ack/s |
...
ClickHouse 싱크는 Flink 체크포인트와 2PC로 묶이지 않는다. 그래서 마지막 구간은
**at-least-once + 읽기 시점 멱등**이다. `ReplacingMergeTree`는 **머지 시점**에
...
### S-8 ← P-8. 재생 루프와 중복 주입을 분리해 지속 부하를 만든다
...
재생 루프는 지속 부하를 만들고, 별도 중복 주입은 dedup 동작을 검증한다.

### S-9 ← P-9. 게이트웨이 신원 바인딩 — `vehicle_id`를 인증서에서만 취한다

게이트웨이를 둔 이유의 절반은 **차량 수천 대에 Kafka ACL을 발급·폐기하는 것이 비현실적**이기
때문이다([검토](ingestion-design-review.md) §4.6). 노출면을 게이트웨이 하나로 줄였으니, 그
하나에 문지기를 세워야 의미가 생긴다.

**인증과 인가는 다른 층이다.**

| | 질문 | 수단 |
|---|---|---|
| 인증 | 이 연결이 우리 fleet의 차량인가 | mTLS — 사설 CA 발급 클라이언트 인증서 **필수**(`client-auth=REQUIRE`) |
| **인가·신원 바인딩** | 이 차량이 **자기 자신의** `vehicle_id`로 쏘고 있는가 | 인증서 SAN URI에서 추출 |

mTLS만으로는 두 번째가 닫히지 않는다. 인증서 하나가 털린 차량 A가 `vehicle_id=B`로 쏘는 것을
막지 못하기 때문이다. 그리고 이 시스템에서 그 결과는 단순 오염이 아니다 — dedup이
`keyBy(vehicle_id) + seq` 슬라이딩 윈도우라(S-7), 남의 `vehicle_id`로 `seq`를 크게 밀면
**진짜 차량 B의 정상 레코드가 윈도우 밖으로 밀려나 `too_old`로 조용히 버려진다.** 표적 수집
거부이고, 이 프로젝트의 핵심 주장인 유실 0을 정면으로 깬다.

**규칙 — 클라이언트가 주장하는 신원을 신뢰하지 않는다.**

1. `vehicle_id`는 **클라이언트 인증서 SAN URI**(`spiffe://fleetsentinel/vehicle/{id}`)에서만 취한다
2. 차량이 스트림 메타데이터로 보낸 `x-vehicle-id`는 **주장**일 뿐이며, 대조해서 다르면
   `PERMISSION_DENIED`로 스트림을 거절하고 지표를 올린다 — 조용히 버리지 않는다
3. Kafka 파티션 키는 **1에서 나온 값**으로 만든다
4. 따라서 `vehicle_id`는 **레코드 페이로드(Avro)에 넣지 않는다**. 두 벌이 있으면 하류가 어느
   쪽을 믿느냐에 따라 구멍이 다시 열린다 — 정본을 하나로 둔다

**공개 PKI가 아니라 사설 PKI다.** 공개 CA는 도메인 소유권만 검증할 수 있어 `vehicle-0042` 같은
이름을 발급해주지 않는다. 검증하는 쪽이 게이트웨이 하나뿐이므로 자체 CA로 충분하고, 그래야
`vehicle_id`를 인증서에 박아 넣을 수 있다.

**닫히지 않는 것**: 같은 차량이 두 스트림을 동시에 여는 **사고 버전**(재연결 경합·LB 재시도)은
그대로 남는다([ack·dedup 설계](ack-dedup-design.md) A-L1). 신원 바인딩은 악의적 변형만 닫는다.
제대로 닫으려면 순서 있는 스트림 세대(epoch)가 필요하다. PKI 수명주기는 L-7.
...
넣을 수 있느냐가 아니라 **넣을 이유가 없다** — SQL로 픽셀을 조회하지 않는다. 관제에서 필요한
것은 클립 메타데이터를 조회하고 원본 파일을 뷰어에서 재생하는 일이다.

남는 질문은 **구조화 계층(1.7%)** 인데, 여기서도 웨어하우스만으로는 부족하다.

| | 웨어하우스 단독 | **오브젝트 스토리지 + ClickHouse (채택)** |
|---|---|---|
| 원시 센서 | 바이너리 적재·반출에 부적합 | 원본 파일 그대로 보관·재생 |
| 실시간 관제 조회 | 별도 서빙 계층이 필요할 수 있음 | ClickHouse에서 시계열·집계 조회 |
| 재처리 | 쿼리 컴퓨트에 종속 | 원본 파일과 Kafka 보존분을 다시 읽음 |
| 운영 구성 | 관리형 서비스 하나 | 저장소와 서빙 DB를 역할별로 운영 |

**기각 이유**: 원시 센서는 오브젝트 스토리지에 두고, 관제용 구조화 데이터는 ClickHouse에서
조회하는 편이 각 데이터 형태와 접근 방식에 맞다. 현재 범위에 별도 분석 웨어하우스를 추가할
질의나 BI 요구사항은 없다.
...
rosbag2 sqlite3는 ROS 밖 생태계가 얇다. 구조화된 관제 데이터는 ClickHouse에 저장하므로
MCAP과 역할이 겹치지 않는다.
...
#### A-14. 수집 게이트웨이를 Spring Cloud Gateway로 구현

"게이트웨이"라는 이름 때문에 API 게이트웨이·리버스 프록시 제품이 후보로 보인다. **기각한다 —
층위가 다르다.**

Spring Cloud Gateway는 **다운스트림 HTTP 서비스로 요청을 라우팅·필터링하는 리버스 프록시**다.
여기엔 라우팅할 다운스트림이 없다. 종착지가 Kafka 프로듀서고, 핵심 작업이 라우트 필터에 넣을
성질이 아니다.

| 이 게이트웨이가 하는 일 | 프록시로 되는가 |
|---|---|
| gRPC 양방향 스트림 종단 | 통과(proxy)는 되지만 **서빙**은 안 된다 |
| 레코드별 `seq` 추적 → 연속 최고값 산출 | ❌ 응용 상태다 |
| **Kafka 프로듀서 콜백(`acks=all`) 이후에만** 완료 표시 | ❌ 프록시는 "전달했다"까지만 안다 |
| 인증서 SAN ⟷ 주장된 `vehicle_id` 대조 후 파티션 키 결정 | ❌ |

세 번째가 결정적이다. HTTP/2 흐름 제어는 **전송**만 보장하고 "게이트웨이가 Kafka에 안전하게
넣었다"를 말해주지 않는다. CACK이 존재하는 이유가 정확히 그것이므로([ack·dedup 설계](ack-dedup-design.md) §1),
프록시 계층에 둘 수 없다.

**채택**: gRPC 서버를 띄우는 평범한 Spring Boot 애플리케이션. **LB는 이 앞에 별도로 선다**
(L4 패스스루 — mTLS를 게이트웨이가 종단해야 인증서에서 `vehicle_id`를 꺼낼 수 있다. LB에서
TLS를 끊으면 L-15의 신원 바인딩이 성립하지 않는다).

**정직한 단서**: 이 경로에서 Spring Boot가 핫패스에 기여하는 것은 없다. 실제 I/O는 grpc-java의
Netty 전송이 하고 Spring은 DI·설정·관측(액추에이터/마이크로미터) 래퍼다. 게이트웨이를 Spring으로
하는 근거는 A-10과 같은 결(운영 일관성·포트폴리오 목적)이지 성능 근거가 아니다.

#### A-15. 실시간 판정 이벤트를 API로 전달하는 경로

| 경로 | 지연 | API 장애 시 Flink 영향 | 재생 | 판단 |
|---|---:|---|---|---|
| Flink → API HTTP | 낮음 | 재시도와 backpressure가 판정 잡에 전파됨 | 별도 구현 | 기각 |
| Flink → ClickHouse → API polling | polling 주기만큼 추가 | 없음 | 가능 | 기각 |
| **Flink → Kafka → API → SSE** | Kafka 전송 + SSE | **없음** | **Kafka retention** | **채택** |

ODD 이탈은 판정 직후 화면에 보여야 한다. Flink가 API를 직접 호출하면 API
배포나 장애가 스트림 잡의 처리율에 영향을 주고, ClickHouse를 polling하면 이미 Kafka에 있는
이벤트를 다시 조회해야 한다. 판정 결과를 저빈도 `fleet.alerts` 토픽에 쓰고 API가 소비하는
구성이 두 문제를 피한다.

알림 Kafka 싱크는 `EXACTLY_ONCE`가 아니라 `AT_LEAST_ONCE`다. 현재 checkpoint 간격은 10초이고,
Kafka transaction을 쓰면 checkpoint가 끝날 때까지 소비자에게 이벤트가 보이지 않는다. 대신
Flink가 `vehicle_id`, 이벤트 종류, 원본 `boot_id`·`seq` 또는 이벤트 시각으로 안정적인
`eventId`를 만들고 API와 브라우저가 중복을 제거한다. 토픽은 저빈도라 1파티션으로 두어 전체
순서와 SSE `Last-Event-ID` 재개를 단순하게 유지한다.

API 인스턴스마다 consumer group을 따로 쓴다. 같은 consumer group으로 API를 여러 대 띄우면
파티션이 인스턴스 사이에 나뉘어 각 서버의 SSE 접속자가 일부 알림만 받는다. 인스턴스별 group은
각 서버가 모든 알림을 읽고, 재시작할 때 Kafka retention에서 최근 1,000건 버퍼를 다시 채우게 한다.

**한계**: 현재 알림 채널은 대시보드 SSE까지다. 문자, 메신저, 호출 시스템 연동과 알림 확인·해제
이력은 구현하지 않았다. ODD 경계도 nuScenes 지도 범위를 감싼 사각형이며 실제 ODD 정책의
날씨·조도·도로 유형 조건은 포함하지 않는다.
...
| L-4 | **3D 박스의 출처** | 대시보드의 객체 박스는 nuScenes 주석을 재생한 것이며 차량이 실시간으로 계산한 결과가 아니다 |
| L-5 | **레이더 페이로드 해석** | `.pcd`를 MCAP에 싣기만 하고 파싱·시각화는 미구현 |
| L-6 | **full 데이터셋 규모 검증** | mini(10장면)로만 측정했다. full 1000장면은 48.8GB로 **추정**이며 미검증 |
| **L-7** | **PKI 수명주기가 없다** | 게이트웨이는 mTLS를 **요구**하고 사설 CA로 발급한 인증서를 검증한다(`scripts/gen-certs.sh`). 그러나 **제조 시점 프로비저닝**(TPM·secure element에 개인키 봉인), **로테이션**, **폐기**(CRL/OCSP), 탈취 차량 격리는 없다. 개발용 CA를 스크립트로 굽는 수준이다. MQTT 저주파 채널과 오브젝트 스토리지 접근 제어도 미설계 |
| **L-15** | **신원 바인딩은 게이트웨이에만 있다** | `vehicle_id`를 클라이언트 인증서 SAN URI에서만 취하고 차량이 주장한 값과 대조해 불일치 시 스트림을 거절한다(S-9). 이것이 **없으면 유실 0 주장이 깨진다** — dedup이 `keyBy(vehicle_id) + seq` 윈도우라, 남의 `vehicle_id`로 `seq`를 크게 밀면 **그 차량의 정상 레코드가 `too_old`로 조용히 버려진다**(표적 수집 거부). 같은 구멍의 **사고 버전**이 [ack·dedup 설계](ack-dedup-design.md) A-L1 스트림 fencing이며 **그쪽은 아직 열려 있다** — 신원 바인딩은 악의적 변형만 닫는다. MQTT 저주파 경로에는 이 검사가 없다 |
...
| **L-14** | **exactly-once가 쓰기 시점이 아니라 읽기 시점에 닫힌다** | Flink 체크포인트와 ClickHouse 쓰기는 2PC로 묶이지 않는다(S-7). `ReplacingMergeTree`는 **머지 시점**에 중복을 제거하므로, 머지 전 질의는 `FINAL`이나 `argMax`로 흡수해야 한다. 잊으면 집계가 부풀 수 있다. **해소함** — `infra/clickhouse/001-schema.sql`이 `_raw` 테이블과 별도로 `FINAL`이 적용된 뷰를 만들고 애플리케이션은 뷰만 본다 |
...
| **L-11** | **구조화 계층에 장기 원본이 없다** | 신호·객체 메타데이터는 Kafka 보존 기간이 지나면 ClickHouse만 남는다. ClickHouse를 완전히 재구성하려면 Kafka 장기 보존이나 오브젝트 스토리지의 append-only 구조화 아카이브가 추가로 필요하다 |
...
| **P2** | 스키마 3종 확정 · **레코드 단위** 재생기 → Kafka | N대 동시 재생이 Kafka에 적재 | 구현 |
| **P3** | Flink (dedup·검증·DLQ·ODD) → ClickHouse/Kafka alerts | 차량별 `seq` 결번 0, 체크포인트 성공, ODD 전이 이벤트 검증 | 구현, 종단 측정 대기 |
| **P4** | Claim-Check · 클립 카탈로그 | 경량·중량 경로가 `blob_uri`로 결합 | |
| **P5** | Spring Boot 4 API — ClickHouse telemetry SSE + Kafka alert SSE | 실 데이터와 판정 이벤트가 같은 화면에 뜬다 | 구현, 종단 측정 대기 |
...
| ClickHouse 테이블 | Kafka 보존분에서 다시 적재한다. 보존 기간 밖의 재구성은 L-11이 열려 있다 |
| 클립 카탈로그 | `segment-ref`와 오브젝트 스토리지 원본을 대조해 다시 만든다 |
...
| 저장 | Kafka → ClickHouse, 원시 센서는 오브젝트 스토리지 | Prometheus |
...
| ODD 이탈 | `ODD_EXITED` 뒤 `ODD_RETURNED`가 없는 차량 수 | > 0 |
...
| 전송 | pytest — CACK·역압·**SIGKILL 후 결과적 exactly-once**(재전송량 예측 대조) | **13건 통과** |
| 프론트 | vitest — 좌표 가드·링버퍼·ODD 상태 전이·중복 제거 | **29건 통과** |
| 포맷 | MCAP 되읽기 — 인덱스·내장 스키마·메시지 수·구간 랜덤 액세스·매직바이트 | **3장면 × 9항목 통과** |
| 기하 | 박스 안 LiDAR 포인트를 세어 라벨과 대조 | 통과 — 결과는 [데이터 설계 §10.3](data-design.md) |
| 인프라 | `make smoke` — 전 서비스 healthy·토픽·**ClickHouse 질의·지리함수**·MinIO 버킷 | 통과 |
| Flink 판정 | JUnit — ODD 이탈·지속 timer·복귀·늦은 위치 | **25건 전체 통과** |
| API 알림 | JUnit — Flink JSON 계약·poison record 격리·eventId 중복 제거·replay 상한 | **8건 전체 통과** |
| 가용성 | `make ha-demo` — 브로커 하드 kill → 리더 재선출 → 유실 0 | **published 5600 = consumed 5600** |
| 정합 | 차량별 `seq` 연속성 (ClickHouse ∪ DLQ에 결번 없음) | 종단 측정 대기 |
| 부하 | 가설 우선 — 목표 RPS·p99 SLO를 **런 전에 봉인**하고 결과와 대조 | 종단 측정 대기 |
...
**[데이터 설계 §10.3](data-design.md)** 이 정본이다.
...
데이터 측면의 미검증 항목은 **[데이터 설계 §10.4](data-design.md)** 에 있다.
Kafka→Flink→ClickHouse와 Kafka alert→API→SSE는 구현됐지만 종단 처리량·탐지 지연·복구
시나리오는 아직 측정하지 않았다. 따라서 블로그의 테스트 및 결과 절은 이 측정 전까지 비워 둔다.
