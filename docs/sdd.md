...
> 상세는 [데이터 설계 §2.1.2](data-design-v3.md).
...
대역폭은 **58배** 차이인데 메시지 건수는 오히려 신호가 **8배** 많다. 하나의 파이프라인으로
...
**기각 이유**: §3 S-3의 표 참조. 스트림 500개에서 647,610 msg/s이고 342바이트 페이로드에
대해 브로커 오버헤드가 지배적이다. 배치의 대가는 **최대 100ms 지연 추가**이며, 관제 요구
