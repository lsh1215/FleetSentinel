...
| 관련 문서 | [데이터 설계](data-design.md) · [실행 절차](../RUN.md) · [수집 계층](../exploration/README.md) |
...
| **L-13** | **유실 방지의 핵심 부분을 구현하지 않는다** | 축적 창을 닫는 **WAL**과 gRPC 스트림의 **ack 프로토콜**이 둘 다 온보드 소프트웨어이고 L-2로 스코프 밖이다. 따라서 **"축적 창을 닫았다"가 아니라 "닫는 방법을 설계했다"** 가 정확한 표현이다. 재생기가 같은 머신에 있어 실제로 문제는 생기지 않지만, 이 구분이 무너지면 검증되지 않은 주장이 된다 — [검토](ingestion-design-review.md) §4.8.6 |
