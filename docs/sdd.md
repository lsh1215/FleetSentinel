...
| **L-13** | **유실 방지의 핵심 부분을 구현하지 않는다** | 축적 창을 닫는 **WAL**과 gRPC 스트림의 **ack 프로토콜**이 둘 다 온보드 소프트웨어이고 L-2로 스코프 밖이다. 따라서 **"축적 창을 닫았다"가 아니라 "닫는 방법을 설계했다"** 가 정확한 표현이다. 재생기가 같은 머신에 있어 실제로 문제는 생기지 않지만, 이 구분이 무너지면 검증되지 않은 주장이 된다. 설계 상세는 [WAL 설계](wal-design.md) — 그 과정에서 **플래시 내구성이 새 제약으로 드러났다**(5년 누적 쓰기가 32GB eMMC 내구를 초과) |
