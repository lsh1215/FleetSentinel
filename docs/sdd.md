...
센서 프레임, 인지 박스는 글로벌 프레임이라 정렬되지 않는다([데이터 설계 §7.2](data-design.md)).
...
### S-11 ← P-11. 게이트웨이 신원 바인딩 — `vehicle_id`를 인증서에서만 취한다
...
4. 따라서 `vehicle_id`는 **레코드 페이로드(Avro)에 넣지 않는다**. 두 벌이 있으면 하류가 어느
   쪽을 믿느냐에 따라 구멍이 다시 열린다 — 정본을 하나로 둔다(S-10과 같은 원칙)
...
| **L-14** | **exactly-once가 쓰기 시점이 아니라 읽기 시점에 닫힌다** | Iceberg 보류로 Flink→싱크 구간의 2PC 원자 커밋을 잃었다(S-7). ClickHouse `ReplacingMergeTree`는 **머지 시점**에 중복을 제거하므로, 머지 전 질의는 `FINAL`이나 `argMax`로 흡수해야 한다. 잊으면 **집계가 조용히 부풀어오른다** — 유실보다 찾기 어려운 종류의 오류다. **해소함** — `infra/clickhouse/001-schema.sql` 이 `_raw` 테이블과 별도로 `FINAL` 이 박힌 뷰를 만들고 애플리케이션은 뷰만 본다. 잊을 수 있는 것을 잊을 수 없게 만든 것이다. Iceberg 도입은 보류 유지 |
