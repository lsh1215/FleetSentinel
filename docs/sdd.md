...
확정된 요구사항은 둘뿐이다 — **유실률 0**(`event_id` 집합 대사로 증명, R-1)과
**관제 조회 p99 < 300ms**(목표치).
...
| dedup | `keyBy(vehicle_id)` + **레코드 단위** 판정 (배치 단위는 [재검토](ingestion-design-review.md) §4.1로 무효), 상태 TTL 10분 |
| Iceberg | 체크포인트 단위 2PC 원자 커밋 — 부분 쓰기 없음 |
| ES | `doc_id=event_id` 멱등 upsert |

**dedup 키·단위를 v2.0에서 바꿨다.** 원래 `keyBy(event_id)` + 레코드 단위였는데 둘 다 틀렸다.

- **키** — 중복은 항상 같은 차량에서 온다(MQTT 재전송·프로듀서 재시도·체크포인트 재생 모두).
  차량 안에서만 확인하면 충분하므로 키 카디널리티가 **수억 → 차량 수**로 줄고, 차량별 순서가
  유지되며, 소스 파티셔닝과 일치해 셔플을 없앨 수 있다.
- **단위** — 중복이 발생하는 단위는 메시지(=배치)다. 배치 단위로 판정하면 상태가
  **132배 작아진다**(N=500·TTL 30분에서 74.6GB → 0.56GB). 배치가 결정적이기 때문에
  성립한다 — 창 경계를 절대 시각에 정렬한 §3 S-3 계약이 여기서 값어치를 한다.
- **방어 2층** — Flink가 배치 중복을 걸러내고, 저장 계층이 `event_id` 유일성으로 새어나온
  것을 잡는다. `event_id`는 여전히 레코드 단위 멱등키다.

상세와 상태 크기 계산은 [`pipeline-notes-provisional.md`](pipeline-notes-provisional.md) §6-1.

**증명 방법**: `event_id` 집합 대사. 발행 집합이 저장분과 DLQ의 합집합에 포함되는지 확인한다.
...
| **L-13** | **WAL은 재생기에서만 검증됐다** | 축적 창을 닫는 WAL을 재생기에 구현해 검증했다 — **SIGKILL 후 `seq` 결번 0**, 처리량 85,836 rec/s(목표의 66배), 테스트 11건([WAL 설계 §3.10](wal-design.md)). 다만 실차량 온보드 소프트웨어는 L-2 범위 밖이므로 **"재생기에서 검증했다, 실차량에는 없다"** 가 정확한 표현이다. 또한 SIGKILL은 프로세스 종료를 재현하고 **전원 손실 창(그룹 커밋 10ms = 13 레코드)은 남는다**. gRPC **ack 프로토콜은 아직 미설계**다. 설계 과정에서 **플래시 내구성이 새 제약으로 드러났다**(5년 누적이 32GB eMMC 내구 초과) |
| **L-12** | **실차량 경로에서 유실을 탐지할 수 없다** | "유실 0"을 `event_id` 집합 대사로 증명하는데, 그 증명은 **발행 집합을 아는 재생에서만** 성립한다. 전송 계층이 조용히 버리면 그것이 존재했다는 사실 자체를 모른다. `event_id`는 유일성을 주지만 **연속성을 주지 않는다**. 해소책은 차량별 단조 순번(`seq`) 추가 — [수집 계층 설계 검토](ingestion-design-review.md) §4.2 |
...
설계 측면에서는 파이프라인 자체가 미구현이므로 처리량·지연·복구 시나리오가 전부 미검증이다.
Elasticsearch·Kibana 기동도 확인하지 않았다.
