...
| | 메시지/초/대 | 메시지 크기 | 500대 합계 |
|---|---|---|---|
| 원시 전송 | 1,468 | ~180 B | 734,080 msg/s |
| **100ms 배치** | **10** | **~27 KB** | **5,000 msg/s** |

`event_id`는 **레코드 단위로 유지**한다 — 배치는 전송 단위일 뿐이고 멱등성의 단위가
아니다. `zoesensors` 955Hz는 배치 안에서 그대로 싣되, 필요하면 다운샘플 비율을
설정으로 노출한다(기본값은 무손실).
...
| L-10 | **MCAP 스윕 누락(현존 결함)** | 현재 변환기는 키프레임만 담는다. `sample_data` 31,206건 중 4,848건(15.5%), 바이트로는 13.6%. **§9 무손실 원칙 위반이며 P2에서 수정 대상** |

L-10은 **알려진 미해결 결함**이므로 숨기지 않고 남긴다.
...
1. L-10 MCAP 스윕 누락 수정 — 세그먼트 크기가 `log-segment` 스키마의 분할 단위를 좌우
2. 신호 배치 정책 확정 — 100ms 창, `zoesensors` 다운샘플 여부
3. NFR 재산정 — v2.0의 5,000 rps는 OBD(1건/초/대) 기준이라 **1,468배 어긋난다**
