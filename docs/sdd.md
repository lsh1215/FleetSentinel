...
| 관련 문서 | [데이터 설계](data-design-v3.md) · [실행 절차](../RUN.md) · [수집 계층](../ingestion/README.md) |

> 이 문서의 모든 수치는 **실측값**이다. nuScenes mini 10장면(196.5초)을 전수 측정해 얻었고,
> 측정 스크립트는 `ingestion/scripts/`에 있다. 추정치는 "추정"이라고 명시한다.
...
v2.0의 NFR(5,000 rps)은 OBD 텔레메트리(**1건/초/대**) 기준이라 v3.0에서 **1,295배**
어긋난다. 아래는 실측 기반으로 다시 잡은 값이다.

| 항목 | 값 | 근거 |
|---|---|---|
| 차량당 신호 발생률 | **1,295 rec/s** | 3장면 60초 실전 추출 |
| 차량당 신호 대역폭 | 342 B/rec × 1,295 = **432 KB/s** | 〃 |
| 차량당 원시 센서 | **27.15 MB/s**, 159 파일/s | mini 전수 |
| **배치 후 Kafka 입력** | **9.8 msg/s/대** (43.4 KB/msg, 131.5 rec/msg) | 100ms 창 실측 |
| **목표 fleet** | **500대** | |
| **Kafka 처리량 목표** | **4,925 msg/s · 648,000 rec/s · 212 MB/s** | 500대 환산 |
| 오브젝트 스토리지 유입 | 13.6 GB/s (트리거 클립이므로 실제는 훨씬 낮음) | 500대 환산 |
| 수집 지연 p99 | `ingest_time - sensor_time` < **1초** | 배치 100ms + 전송 여유 |
| 관제 조회 p99 | < 300ms (Kibana/ES) | v2.0에서 승계 |
| 유실률 | **0** — `event_id` 집합 대사로 증명 | R-1 |
| 중복률 | Silver 기준 0 (Kafka 계층은 at-least-once 허용) | S-7 |

**창 크기 트레이드오프** (실측, 500대 환산):
...
**100ms를 채택한다.** 메시지 수가 1/131로 줄고, 추가 지연 100ms는 관제 요구(R-3)에서
무시 가능하며, 메시지 크기 43.4 KB가 Kafka 기본 설정(`max.message.bytes` 1MB) 안에
넉넉히 들어간다. 창을 더 키우면 지연이 선형으로 늘 뿐 얻는 것이 적다.
...
신호를 100ms 창으로 묶어 하나의 Kafka 메시지로 만든다.

| | 메시지/초/대 | rec/msg | 메시지 크기 | 500대 합계 |
|---|---|---|---|---|
| 원시 전송 | 1,295.2 | 1.0 | 342 B | 647,610 msg/s |
| **100ms 배치** | **9.8** | **131.5** | **43.4 KB** | **4,925 msg/s** |

배치 계약 3가지를 고정한다.

1. **멱등성의 단위는 레코드다.** `event_id`는 레코드마다 유지되고 배치는 전송 단위일 뿐이다.
   Flink는 배치를 펼친 뒤 `keyBy(event_id)`로 dedup한다.
2. **창 경계는 `sensor_time` 기준**이다(`ingest_time`이 아니다). 재전송·지연이 있어도
   같은 레코드는 항상 같은 창에 들어간다 — 결정적이어야 재현이 된다. 창은 epoch 절대
   시각에 정렬한다(`window_start = (sensor_time // window) * window`).
3. **부분 창을 버리지 않는다.** 마지막 창이 덜 찼어도 그대로 방출한다(무손실).

**`zoesensors` 955Hz는 다운샘플하지 않는다.** 신호 계층에도 무손실 원칙이 적용된다 —
`ego_pose` 시각에 최근접 결합하는 방식(20Hz)을 쓰면 이 채널의 **98%가 버려진다**.
채널 네이티브 주기 그대로 싣고, 대신 배치로 메시지 수를 흡수한다.
