...
> 면접관이 이 대시보드를 보고 **파고들 만한 기술적 지점**을 정리한다. 총 25항목.
...
**검증 상태** — 순수 로직 단위 테스트 **29건**(`npm test`), 타입 체크·프로덕션 빌드 통과,
SSE 스트림·재개 실측 확인. GL·캔버스 렌더는 육안 확인이 필요하다.

**전제가 되는 수치** — 차량 4대 재생 기준 **초당 약 5,100 레코드**가 들어오고, SSE는 이를
**100ms 프레임(초당 약 39건)** 으로 묶어 보낸다. 실측값이며
[데이터 설계 §4](data-design.md)이 정본이다. 이 숫자가 아래 대부분의 판단을 만든다.

> **묶는 지점이 어디인지가 중요하다.** 차량→클라우드 구간에서는 애플리케이션 배치를
> **폐기했다** — 축적 창만큼의 유실이기 때문이다([수집 설계 검토](ingestion-design-review.md) §4.1).
> 브라우저 구간의 100ms 프레임은 그것과 **다른 홉의 다른 결정**이다. 이유는 Q3-1.
...
### Q3-1. 차량 구간에서는 배치가 유실이라며 폐기했는데, 왜 브라우저로는 배치로 보내나?

**왜 묻는가** — 같은 프로젝트 안에서 정반대 결정을 두 번 했다. 근거가 있으면 홉마다
요구가 다르다는 것을 이해한 것이고, 없으면 한쪽이 베낀 것이다.

**답** — 두 홉의 **실패 비용이 다르다.**

| | 차량 → 클라우드 | API → 브라우저 |
|---|---|---|
| 창 안에서 죽으면 | **레코드가 영구 소멸** — 프로토콜에 들어간 적이 없어 재전송 대상도 아니고 결번도 안 남는다 | ClickHouse에 이미 있다. `Last-Event-ID`로 다시 받는다 |
| 데이터의 정체 | **유일한 원본** | **원본의 사본** |
| 되돌릴 수 있나 | 불가능 | 언제든 재질의 |

차량 쪽은 그 레코드가 세상에 하나뿐이라 축적 창이 곧 유실 창이다. 브라우저 쪽은
**뷰**이고, 놓치면 다시 물어보면 된다. 그래서 같은 "100ms 묶기"가 한쪽에서는 데이터
유실이고 다른 쪽에서는 그냥 렌더 예산 절약이다.

**되묻는다면 — 배치를 안 하면 어떻게 되나?**

레코드 단위로 SSE를 보내면 초당 39건이 아니라 **5,100건**이 된다. 수신 경로에
`setState`가 없으므로(Q4) 저장소 구조는 그대로 버티지만, **메시지당 고정비**가 39배로
늘어난다 — `JSON.parse` 호출, `EventSource` 이벤트 디스패치, 프레임 헤더. 지금 배치
하나가 43 KB인데(§10 「안 한 것」) 레코드 하나는 약 340 B라 파싱 비용 대비 오버헤드 비율이 나빠진다.

즉 브라우저 구간에서 배치는 **얻는 것만 있고 잃는 것이 없다.** 차량 구간에서는 잃는
것이 있었고, 그게 결정을 뒤집은 이유다.

📄 `server/mockStream.ts` — 픽스처가 `batches.json`인 것이 이 결정의 흔적이다
...
객체 3D 박스는 **글로벌 ENU 미터 좌표**로 온다(`ego_pose`와 같은 프레임). 지도는 WGS84
위경도를 요구하므로 변환이 필요하다.
...
**③ 프론트와 백엔드가 같은 원점을 써야 한다** — 어긋나면 **차량과 객체 메타데이터가 서로 다른
위치에 찍힌다.** 값이 조금만 달라도 박스가 도로 옆에 떠 있게 되어 센서와 객체 좌표가
어긋난다. 테스트로 고정했다.
...
| LiDAR 포인트 0개 객체는 **점선 + 낮은 불투명도** | 원천 관측 상태를 숨기지 않으면서 다른 객체와 구분한다. 색에만 의존하지 않도록 점선을 함께 쓴다 |
